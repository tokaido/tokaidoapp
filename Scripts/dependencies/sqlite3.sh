lib="$TKD_RUBY_SQLITE3";
version="$TKD_RUBY_SQLITE3_VERSION";
...
if [ -f "$TKD_DEPENDENCIES_TARBALLS/$lib-$version.tar.gz" ]; then
  ...
  curl -o "$lib-$version.tar.gz" "https://www.sqlite.org/2019/$lib-$version.tar.gz"
  cd $TKD_HOME_DEV
  echo "Done."
fi
...
rm $TKD_RUBY_SQLITE3_PCONFIG.pc
ln -s ../current/$lib/lib/pkgconfig/$TKD_RUBY_SQLITE3_PCONFIG.pc
