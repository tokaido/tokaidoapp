lib="$TKD_RUBY_LIBFFI";
version="$TKD_RUBY_LIBFFI_VERSION";
...
if [ -f "$TKD_DEPENDENCIES_TARBALLS/$lib-$version.tar.gz" ]; then
  ...
  curl -o "$lib-$version.tar.gz" "https://gcc.gnu.org/pub/libffi/$lib-$version.tar.gz"
  cd $TKD_HOME_DEV
  echo "Done."
fi
...
rm $TKD_RUBY_LIBFFI_PCONFIG.pc
ln -s ../current/$lib/lib/pkgconfig/$TKD_RUBY_LIBFFI_PCONFIG.pc
...
ln -sf ../current/$lib/lib/$lib-$version/include include/$lib
