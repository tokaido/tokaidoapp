lib="$TKD_RUBY_OPENSSL";
version="$TKD_RUBY_OPENSSL_VERSION";
...
if [ -f "$TKD_DEPENDENCIES_TARBALLS/$lib-$version.tar.gz" ]; then
  ...
  curl -o "$lib-$version.tar.gz" "https://www.openssl.org/source/$lib-$version.tar.gz"
  cd $TKD_HOME_DEV
  echo "Done."
fi
...
# OpenSSL specific
mkdir -p $prefix_path/etc/openssl

./Configure darwin64-x86_64-cc enable-ec_nistp_64_gcc_128 --prefix=$prefix_path\
    no-shared\
    no-ssl3\
    no-ssl3-method\
    no-zlib\
    --openssldir="$prefix_path/etc/openssl"
